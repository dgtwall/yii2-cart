...
class CartItem extends \yii\db\ActiveRecord
{
    ...
    /**
     * {@inheritdoc} 为了使用grid-view演示模板格式化的数据结构，理想的数据结构看下面的 formatCartItemsGroupShop 方法
     * @param $cartItems
     * @return array
     */
    ...
    public static function calCartItemsPrice($cartItems)
    {
        ...
        foreach ($shopCartItems as &$shopCartItem) {
            ...
            //计算店铺的满减价格
            $discountShopTotalPrice = self::callDiscountPrice($shopTotalPrice, 'shopFullDiscount');
            //店铺商品优惠后总价格
    ...
    public static function callDiscountPrice($price, $ruleType)
    {
        ...
        if ($ruleType == 'discount') {
            ...
            $discountValue = $price - $price * $rule['discount'];
        }
        ...
        return [
            'dicountValue' => $discountValue,
            'discountPrice' => $discountPrice,
        ];

    }

    public function getDiscountRuleConfig($activityId)
    {
        return [
            //满减:满300减40
            'fullDiscount' => [
